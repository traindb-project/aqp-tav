...
def _resolve_workbench_user(user_id, is_api_key, workbench_id):
    if user_id is not None:
        if is_api_key:
            api_key = user_id
            if workbench_id is not None:
                q = models.Workbench.get_by_id(workbench_id)
            else:
                q = models.Workbench.by_api_key(api_key)
...
# Added by jscho 2023-04-28
class WorkbenchQueryExecutor(object):
    def __init__(
        self, query, data_source_id, user_id, is_api_key, metadata
    ):
        self.query = query
        self.data_source_id = data_source_id
        self.metadata = metadata
        self.data_source = self._load_data_source()
        self.workbench_id = metadata.get("workbench_id")
        self.user = _resolve_workbench_user(user_id, is_api_key, metadata.get("workbench_id"))
        self.query_model = (
            models.Workbench.query.get(self.workbench_id)
            if self.workbench_id and self.workbench_id != "adhoc"
            else None
        )
    ...
    def run(self):
        ...
        logger.debug("Executing workbench query:\n%s", self.query)
        self._log_progress("executing_workbench_query")
        ...
        try:
            ...
            data = None
            logger.warning("Unexpected error while running workbench query:", exc_info=1)

        run_time = time.time() - started_at

        logger.info(
            "job=execute_workbench_query query_hash=%s ds_id=%d data_length=%s error=[%s]",
            self.query_hash,
            self.data_source_id,
            data and len(data),
            error,
        ...
        if error is not None and data is None:
            ...
            query_result = models.WorkbenchQueryResult.store_result(
                self.data_source.org_id,
                self.data_source,
                self.query_hash,
                self.query,
            ...
            models.Workbench.update_latest_result(query_result)
    ...
    def _log_progress(self, state):
        logger.info(
            "execute_workbench_query state=%s query_hash=%s type=%s ds_id=%d "
            "workbench_id=%s username=%s",
            state,
            self.query_hash,
            self.data_source.type,
            self.data_source.id,
            self.metadata.get("workbench_id", "unknown"),
            self.metadata.get("Username", "unknown"),
        )

    def _load_data_source(self):
        logger.info("job=execute_workbench_query state=load_ds ds_id=%d", self.data_source_id)
        return models.DataSource.query.get(self.data_source_id)


# Added by jscho 2023-04-28
def execute_workbench_query(
    query,
    data_source_id,
    metadata,
    user_id=None,
...
class TrainQueryExecutor(object):
    def __init__(
        self, query, data_source_id, user_id, is_api_key, metadata
    ):
        self.query = query
    ...
    def run(self):
        ...
        logger.debug("Executing Train query:\n%s", self.query)
        self._log_progress("executing_train_query")
        ...
        logger.info(
            "job=execute_workbench_query query_hash=%s ds_id=%d data_length=%s error=[%s] run_time%d",
            self.query_hash,
            self.data_source_id,
            data and len(data),
            error,
    ...
    def _log_progress(self, state):
        logger.info(
            "execute_train_query state=%s query_hash=%s type=%s ds_id=%d user_id=%s",
            state,
            self.query_hash,
            self.data_source.type,
            self.data_source.id,
