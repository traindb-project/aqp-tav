...
# Added by jscho - 2023-04-27 : WorkbenchQueryResultPersistence
WorkbenchQueryResultPersistence = (
    settings.dynamic_settings.WorkbenchQueryResultPersistence or DBPersistence
)

# Added by jscho - 2023-04-27 : Workbench Query Result
@generic_repr("id", "org_id", "data_source_id", "query_hash", "runtime", "retrieved_at")
class WorkbenchQueryResult(db.Model, WorkbenchQueryResultPersistence, BelongsToOrgMixin):
    id = primary_key("WorkbenchQueryResult")
    org_id = Column(key_type("Organization"), db.ForeignKey("organizations.id"))
    org = db.relationship(Organization)
    data_source_id = Column(key_type("DataSource"), db.ForeignKey("data_sources.id"))
    data_source = db.relationship(DataSource, backref=backref("workbench_query_results"))
    query_hash = Column(db.String(32), index=True)
    query_text = Column("query", db.Text)
    _data = Column("data", db.Text)
    runtime = Column(postgresql.DOUBLE_PRECISION)
    ...
    __tablename__ = "workbench_query_results"
    ...
    def unused(cls, days=7):
        ...
        return (
            cls.query.filter(
                Workbench.id.is_(None), cls.retrieved_at < age_threshold
            ).outerjoin(Workbench)
        ).options(load_only("id"))
...
# added by jscho - 2023-04-27 : Workbench
@gfk_type
@generic_repr(
    "id",
    "name",
...
class Workbench(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    id = primary_key("Workbench")
    version = Column(db.Integer, default=1)
    org_id = Column(key_type("Organization"), db.ForeignKey("organizations.id"))
    org = db.relationship(Organization, backref="workbenches")
    data_source_id = Column(key_type("DataSource"), db.ForeignKey("data_sources.id"), nullable=True)
    data_source = db.relationship(DataSource, backref="workbenches")
    latest_query_data_id = Column(
        key_type("WorkbenchQueryResult"), db.ForeignKey("workbench_query_results.id"), nullable=True
    )
    latest_query_data = db.relationship(WorkbenchQueryResult)
    name = Column(db.String(255))
    description = Column(db.String(4096), nullable=True)
    query_text = Column("query", db.Text)
    query_hash = Column(db.String(32))
    ...
    last_modified_by_id = Column(key_type("User"), db.ForeignKey("users.id"), nullable=True)
    last_modified_by = db.relationship(
        User, backref="modified_workbenches", foreign_keys=[last_modified_by_id]
    )
    is_archived = Column(db.Boolean, default=False, index=True)
    is_draft = Column(db.Boolean, default=True, index=True)

    # updated by wgkim 2023-08-14 : TrainDB와 유사한 구조로 변경
    modeltype_name = Column(db.String(255))
    schema_name = Column(db.String(255))
    table_name = Column(db.String(255))
    columns = Column(db.String(255))
    table_rows = Column(db.Integer)
    trained_rows = Column(db.Integer)
    model_options = Column(db.String(4096))
    model_server = Column(db.String(255))
    status = Column(db.String(16))
    start_time = Column(db.DateTime(True), default=db.func.now())

    # updated by jscho 2023-04-18 : no visualizations(default visualization)
    visualizations = None
    options = Column(MutableDict.as_mutable(PseudoJSON), default={})
    search_vector = Column(
        TSVectorType(
    ...
    @classmethod
    def create(cls, **kwargs):
        workbench = cls(**kwargs)
        return workbench
    
    # Updated by wgkim 2023-08-03
    @classmethod
    def all_queries(
        cls, group_ids, user_id=None, include_drafts=False, include_archived=False
    ):
        query_ids = (
            db.session.query(distinct(cls.id))
            .join(
                DataSourceGroup, Workbench.data_source_id == DataSourceGroup.data_source_id
            )
            .filter(Workbench.is_archived.is_(include_archived))
            .filter(DataSourceGroup.group_id.in_(group_ids))
        )
        queries = (
            cls.query.options(
                joinedload(Workbench.user),
                joinedload(Workbench.latest_query_data).load_only(
                    "runtime", "retrieved_at"
                ),
            )
            .filter(cls.id.in_(query_ids))
            # Adding outer joins to be able to order by relationship
            .outerjoin(User, User.id == Workbench.user_id)
            .outerjoin(WorkbenchQueryResult, WorkbenchQueryResult.id == Workbench.latest_query_data_id)
            .options(
                contains_eager(Workbench.user), contains_eager(Workbench.latest_query_data)
            )
        )

        if not include_drafts:
            queries = queries.filter(
                or_(Workbench.is_draft.is_(False), Workbench.user_id == user_id)
            )
        return queries

    # Added by wgkim 2023-08-03
    @classmethod
    def by_user(cls, user):
        return cls.all_queries(user.group_ids, user.id).filter(Workbench.user == user)
    ...
    def update_latest_result(cls, query_result):
        # TODO: Investigate how big an impact this select-before-update makes.
        workbenches = Workbench.query.filter(
            Workbench.query_hash == query_result.query_hash,
            Workbench.data_source == query_result.data_source,
        )

        for q in workbenches:
            q.latest_query_data = query_result
            # don't auto-update the updated_at timestamp
            q.skip_updated_at = True
            db.session.add(q)

        workbench_ids = [q.id for q in workbenches]
        logging.info(
            "Updated %s queries with result (%s).",
            len(workbench_ids),
            query_result.query_hash,
...
# added by jscho - 2023-04-27 : Workbench insert/update
@listens_for(Workbench, "before_insert")
@listens_for(Workbench, "before_update")
def receive_before_workbench_insert_update(mapper, connection, target):
    target.update_query_hash()


# added by jscho - 2023-04-27 : Workbench set
@listens_for(Workbench.user_id, "set")
def workbench_last_modified_by(target, val, oldval, initiator):
    target.last_modified_by_id = val
