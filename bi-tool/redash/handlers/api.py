...
from redash.handlers.workbenches import (
    WorkbenchListResource,
    WorkbenchResource,
    MyWorkbenchesResource,
    WorkbenchSearchResource,
)
from redash.handlers.workbench_results import (
    WorkbenchResultResource,
    WorkbenchResultListResource,
)

from redash.handlers.train_models import (
    HyperparametersResource,
    TrainModelExportResource,
    TrainModelImportResource,
    TrainModelTrainingsResource,
    TrainModelTypesResource,
    TrainModelsResource,
    TrainModelResource,
    TrainTargetPreviewResource,
)
...
# Updated by wgkim 2023-10-11 : 이 부분의 
api.add_org_resource(
...
# Added by jscho 2023-07-27
api.add_org_resource(TrainModelsResource, "/api/train_models", endpoint="train_models")
api.add_org_resource(TrainModelResource, "/api/train_models", endpoint="delete_train_models")
api.add_org_resource(TrainModelResource, "/api/train_models/<model_name>", endpoint="train_model")
# Added by jscho 2023-08-24
api.add_org_resource(TrainTargetPreviewResource, "/api/train_target_preview", endpoint="train_target_preview")

# Addeb by wgkim 2023-08-16 : 모델타입 및 하이퍼파라미터 받아오는 API 추가
api.add_org_resource(TrainModelTypesResource, "/api/train_model_types", endpoint="train_model_types")
api.add_org_resource(HyperparametersResource, "/api/hyperparameters", endpoint="default_hyperparameters")

# Added by wgkim 2023-09-07 : 마이그레이션 기능 추가
api.add_org_resource(TrainModelImportResource, "/api/migration/import", endpoint="import")
api.add_org_resource(TrainModelExportResource, "/api/migration/export", endpoint="export")

api.add_org_resource(TrainModelTrainingsResource, "/api/trainings", endpoint="trainings")



# Added by jscho 2023-04-18
## 미리보기 퀴리 수행 순서
# handlers.workbench_results.WorkbenchResultListResource.post()
# -> handlers.workbench_results.WorkbenchResultListResource.run_query()
# -> handlers.workbench_results.run_query()
# -> redash.tasks.queries.execute_workbench_query()
# -> redash.tasks.queries.execution.execute_workbench_query()
# -> redash.tasks.queries.execution.WorkbenchQueryExecutor.run()
# -> query_runner.run_query()
# -> models.WorkbenchQueryResult.store_result()
# -> models.Workbench.update_latest_result()
api.add_org_resource(WorkbenchListResource, "/api/workbenches", endpoint="workbenches")


# Added by wgkim 2023-08-03 : 쿼리 검색 기능 추가
api.add_org_resource(MyWorkbenchesResource, "/api/workbenches/my", endpoint="my_workbenches")
api.add_org_resource(WorkbenchResultListResource, "/api/workbench_results", endpoint="workbench_query_results")
api.add_org_resource(WorkbenchSearchResource, "/api/workbenches/search", endpoint="workbenches_search")
api.add_org_resource(WorkbenchResultResource, "/api/workbenches/<workbench_id>/results/<workbench_query_result_id>.<filetype>", endpoint="workbench_query_result")
api.add_org_resource(WorkbenchResource, "/api/workbenches/<workbench_id>", endpoint="workbench")
