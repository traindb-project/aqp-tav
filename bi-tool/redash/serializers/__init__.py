...
class WorkbenchSerializer(Serializer):
    def __init__(self, object_or_list, **kwargs):
        self.object_or_list = object_or_list
        self.options = kwargs

    def serialize(self):
        if isinstance(self.object_or_list, models.Workbench):
            result = serialize_workbench(self.object_or_list, **self.options)
            if (
                self.options.get("with_favorite_state", True)
                and not current_user.is_api_user()
            ):
            ...
            result = [
                serialize_workbench(workbench, **self.options) for workbench in self.object_or_list
            ]
            if self.options.get("with_favorite_state", True):
                favorite_ids = models.Favorite.are_favorites(
                    current_user.id, self.object_or_list
...
# Added by jscho 2023-04-18
# Updated by wgkim 2023-08-14
def serialize_workbench(
    workbench,
    with_stats=False,
    with_visualizations=False,
    with_user=True,
    ...
    d = {
        "id": workbench.id,
        "latest_query_data_id": workbench.latest_query_data_id,
        "name": workbench.name,
        "description": workbench.description,
        "query": workbench.query_text,
        "query_hash": workbench.query_hash,
        "api_key": workbench.api_key,
        "is_archived": workbench.is_archived,
        "is_draft": workbench.is_draft,
        ...
        # "table_name": workbench.table_name,
        # "column_name": workbench.column_name,
        # "train_model_type_id": workbench.train_model_type_id,
        # "train_status_cd": workbench.train_status_cd,

        "modeltype_name": workbench.modeltype_name,
        "schema_name": workbench.schema_name,
        "table_name": workbench.table_name,
        "columns": workbench.columns,
        "table_rows": workbench.table_rows,
        "trained_rows": workbench.trained_rows,
        "model_options": workbench.model_options,
        "model_server": workbench.model_server,
        "status": workbench.status,
        "start_time": workbench.start_time,
    }

    if with_user:
        d["user"] = workbench.user.to_dict()
    else:
        d["user_id"] = workbench.user_id

    if with_last_modified_by:
        d["last_modified_by"] = (
            workbench.last_modified_by.to_dict()
            if workbench.last_modified_by is not None
            else None
        )
    else:
        d["last_modified_by_id"] = workbench.last_modified_by_id

    if with_stats:
        if workbench.latest_query_data is not None:
            d["retrieved_at"] = workbench.retrieved_at
            d["runtime"] = workbench.runtime
        else:
            d["retrieved_at"] = None
            d["runtime"] = None
    ...
    if with_visualizations:
        d["visualizations"] = [
            serialize_visualization(vis, with_query=False)
            for vis in workbench.visualizations
        ]
