...
# Commented by wgkim 2023-04-17 2-4
# 팩토리 함수는 신규 무언가를 만드는데 기능만 모여있고, 기존의 데이터를 불러오는 기능은 포함되어있지 않음
# return 타입은 모델폴더의 User와 같은 클래스이기에, 직접설정하거나 쿼리에서 가져와서 사용가능할듯 
class Factory(object):
    def __init__(self):
        # Deleted by wgkim 2023-04-17 2-6 : 중복된 키로인해 테스트 진행이 되지 않음
        # self.org, self.admin_group, self.default_group = redash.models.init_db()
        self.org, self.admin_group, self.default_group = [None, None, None]
