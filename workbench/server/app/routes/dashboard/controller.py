...
@router.post("")
async def create_dashboard(dto: CreateDashboardDto, db: Session = Depends(get_system_db)):
...
async def update_dashboard(dashboard_id: int, dto: UpdateDashboardDto, db: Session = Depends(get_system_db)):
