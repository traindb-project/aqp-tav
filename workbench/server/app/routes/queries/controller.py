...
async def find_queries(
    ...
    return db.query(Query).filter(Query.traindb_id == traindb_id).order_by(Query.updated_at.desc()).all()


@router.get("/{query_id}")
async def find_query(query_id: int, db: Session = Depends(get_system_db)) -> FindQueryDto:
    return db.query(Query).filter(Query.id == query_id).first()


@router.get("/{query_id}/run")
async def run_query(query_id: int, db: Session = Depends(get_system_db)) -> RunQueryResponseDto:
    ...
    query = query.sql[:-1] if query.sql[-1] == ';' else query.sql
...
@router.put("/{query_id}")
async def update_query(query_id: int, dto: UpdateQueryDto, db: Session = Depends(get_system_db)):
