...
@router.get("")
async def find_databases(traindb_id: int, db: Session = Depends(get_system_db)) -> List[FindDatabaseDto]:
    return db.query(Database).filter(Database.traindb_id == traindb_id).order_by(Database.updated_at.desc()).all()


@router.get("/{database_id}")
async def find_database(database_id: int, db: Session = Depends(get_system_db)) -> FindDatabaseDto:
...
async def test_connection(dto: TestDatabaseConnectionRequestDto) -> TestDatabaseConnectionResponseDto:
